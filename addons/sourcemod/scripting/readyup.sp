...
{
	conf = LoadGameConfigFile("left4downtown.l4d2");
...
RestartCampaignAny()
{
	...
	new Handle:func = EndPrepSDKCall();
...
bool:ShouldResetRoundTwiceToGoLive()
{
	...
	decl String:ScavRest[10];
	GetConVarString(cvarScavRestart, ScavRest, sizeof(ScavRest));
	if(!g_bIsStarted && IsScavengeMode() && StrEqual(ScavRest, "1", true) && round == 1) //scavenge pre-first round warmup
	return true;
